...
from config import mouse, keyboard, Key
...
class HandControl(ChangeLevel):
    ...
    def ClearDrawings(self):
        self.BigArray.append(self.Array)
        self.Array = []
        self.BigArray = []

    def DrawLetter(self, img):
        for i in range(len(self.Array) - 1):
            cv.line(img, (self.Array[i][0], self.Array[i][1]), (self.Array[i + 1][0], self.Array[i + 1][1]),
                    (255, 255, 255), 5)
        for j in self.BigArray:
            for i in range(len(j) - 1):
                cv.line(img, (j[i][0], j[i][1]), (j[i + 1][0], j[i + 1][1]),
                        (255, 255, 255), 5)
